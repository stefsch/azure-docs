...
| Naked Domain Host Name | Specify the root domain here.  The issued certificate will secure *both* the root domain and the `www` subdomain.  The issued certificate will contain the root domain in the certificate's Common Name field, and the `www` subdomin in the Subject Altnernative Name field. |
